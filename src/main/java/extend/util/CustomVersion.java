...
public class CustomVersion {
    ...
    protected static void parseVersion(String version) {
        ...
        if (splitversion.length > 0) {
            MAJOR_VERSION = Util.parseIntDefault(splitversion[0], -1);
        }
        if (splitversion.length > 1) {
            MINOR_VERSION = Util.parseIntDefault(splitversion[1], -1);
        }
        if (splitversion.length > 2) {
            REVISION_VERSION = Util.parseIntDefault(splitversion[2], -1);
        }
        if (splitversion.length > 3) {
            RELEASE_NUMBER = Util.parseIntDefault(splitversion[3], -1);
        }
    }
